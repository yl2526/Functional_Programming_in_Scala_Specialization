...
trait Solver extends GameDef {
  ...
  b.legalNeighbors.map(pbm => (pbm._1, pbm._2::history)).toStream
  ...
  def from(initial: Stream[(Block, List[Move])],
    ...
    case Empty => Empty
    case initPair #:: rest => {
      val newExplored = explored + initPair._1
      val newNeightbors = newNeighborsOnly(neighborsWithHistory(initPair._1, initPair._2), explored)
      initPair #:: from(rest #::: newNeightbors, newExplored)
    }
  }
  ...
  /**
    ...
    */
  lazy val pathsToGoal: Stream[(Block, List[Move])] = pathsFromStart.filter(pbm => done(pbm._1))
