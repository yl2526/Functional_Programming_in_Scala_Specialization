...
class ParallelCountChangeSuite extends FunSuite {
  ...
  test("moneyThreshold") {
    val threshold9 = moneyThreshold(9)
    assert(threshold9(6, List()))
    assert(!threshold9(7, List()))

    val threshold10 = moneyThreshold(9)
    assert(threshold10(6, List()))
    assert(!threshold10(7, List()))
  }
  ...
  test("countChange should return 0 for money < 0") {
    ...
    def checkPar(money: Int, coins: List[Int]) =
      assert(parCountChange(money, coins, moneyThreshold(money)) === 0,
        s"countChang($money, _) should be 0")

    checkPar(-1, List())
    checkPar(-1, List(1, 2, 3))
    checkPar(-Int.MinValue, List())
    checkPar(-Int.MinValue, List(1, 2, 3))
  }
