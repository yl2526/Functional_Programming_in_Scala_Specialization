...
import FloatOps._
  ...
  test("leaf insert with size 0 and minimumSize of 1e-5") {
    val quad = Empty(51f, 46.3f, 0f)
    val b1 = new Body(3f, 54f, 46f, 0f, 0f)
    val b2 = new Body(123f, 18f, 26f, 0f, 0f)
    val inserted = quad.insert(b1).insert(b2)
    inserted match {
      case Leaf(centerX, centerY, size, bodies) =>
        assert(centerX === 51f)
        assert(centerY === 46.3f)
        assert(size === 0)
        assert(bodies === Seq(b1, b2))
      case _ =>
        fail("size less than minimumSize should have returned a Leaf")
    }
  }
  ...
  test("leaf insert with size 5 and minimumSize of 1e-5") {
    ...
    inserted match {
      ...
      case _ =>
        fail("size less than minimumSize should have returned a Fork")
    }
  }
