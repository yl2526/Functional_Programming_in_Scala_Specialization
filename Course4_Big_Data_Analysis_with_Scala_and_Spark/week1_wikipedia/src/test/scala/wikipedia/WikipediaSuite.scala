...
class WikipediaSuite extends FunSuite with BeforeAndAfterAll {
  ...
  test("'occurrencesOfLang' should work for (specific) RDD with one element") {
    ...
    assert(occurrencesOfLang("Java", rdd) === 1, "occurrencesOfLang given RDD JAVA should equal to 1")
  }

  test("'occurrencesOfLang' should work for (specific) RDD with one element Javascript") {
    assert(initializeWikipediaRanking(), " -- did you fill in all the values in WikipediaRanking (conf, sc, wikiRdd)?")
    import WikipediaRanking._
    val rdd = sc.parallelize(Seq(WikipediaArticle("title", "Javascript")))
    assert(occurrencesOfLang("Java", rdd) === 0, "occurrencesOfLang given RDD JAVAscript should equal to 1")
  }
