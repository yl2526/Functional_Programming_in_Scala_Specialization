...
object TimeUsage {
  ...
  def dfSchema(columnNames: List[String]): StructType =
    StructType(StructField(columnNames.head, StringType, false) :: columnNames.tail.map(StructField(_, DoubleType, false)))
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    def category(act: String): Int =
      if (List("t01", "t03", "t11", "t1801", "t1803").exists(prefix => act startsWith prefix)) 1
      else if (List("t05", "t1805").exists(prefix => act startsWith prefix)) 2
      else 3

    val colGroup = columnNames
      .map(colName => (category(colName), new Column(colName)))
      .groupBy(_._1)
      .map { case (cate, col) => cate -> col.map(_._2) }
  ...
  def timeUsageSummaryTyped(timeUsageSummaryDf: DataFrame): Dataset[TimeUsageRow] = {
    //timeUsageSummaryDf.as[TimeUsageRow] should work. Don't know why getAs is necessary. Am I using it in a wrong way?
    timeUsageSummaryDf.map(row => new TimeUsageRow(
  ...
  def timeUsageGroupedTyped(summed: Dataset[TimeUsageRow]): Dataset[TimeUsageRow] = {
    ...
    summed
      ...
      .agg(round(avg[TimeUsageRow](_.primaryNeeds), 1).alias("primaryNeeds").as[Double],
           round(avg[TimeUsageRow](_.work), 1).alias("work").as[Double],
           round(avg[TimeUsageRow](_.other), 1).alias("other").as[Double])
      .map(p => new TimeUsageRow(p._1._1, p._1._2, p._1._3, p._2, p._3, p._4))
      .orderBy($"working", $"sex", $"age")
  }
